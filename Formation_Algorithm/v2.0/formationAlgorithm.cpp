...
/*Differences from v1:
	Compensates for the initial square being beyond the field boundary
*/
...
void Formation::squareCalc(Coordinate centroid, int squareSize, Coordinate Square[4]){
  ...
  for(int i=0;i<4;i++){
	  if(Square[i].x <0 || Square[i].y<0){
		  Serial.println("ErrorErrorError");
	  }
  }
}
...
void Formation::squareOptimization(){
  ...
  for(int i=0;i<4;i++){
	  if(tempSquare[i].x <0 || tempSquare[i].y<0){
		 wrongSquare = true;
	  }
  }
  Serial.println("TempSquare");
  ...
  if(wrongSquare){
		Serial.println("adfklja;dlfkjasd;flkajsdf;lkasjdf;asldkjfa;slkdfja;sdlkfja;");
  for (int i = 0; i < initialLongestDistance; i++) {
	...
	for(int i=0;i<4;i++){
		// Serial.println("Beginning of TempSquare");
		// Serial.println(tempSquare[i].x);
		// Serial.println(tempSquare[i].y);
    ...
    for (int m = 0; m < 4; m++) {
      robots[m].distToTarget = dist(tempSquare[robots[m].targetPt], robots[m].robotPoint);
	  Serial.println("Robot's distToTarget");
	  Serial.println(robots[m].distToTarget);
    }
    qsort(robots, 4, sizeof(Robot), comparedistToTarget);
    if (ct==4 ) {
	...
	Serial.println("Fresh off the boat");
	  for(int m=0;m<4;m++){
		  m_point[m][0] = tempSquare[robots[m].targetPt].x;
		  m_point[m][1] = tempSquare[robots[m].targetPt].y;
		  Serial.println(m_point[m][0]);
		  Serial.println(m_point[m][1]);
	  }
    }
