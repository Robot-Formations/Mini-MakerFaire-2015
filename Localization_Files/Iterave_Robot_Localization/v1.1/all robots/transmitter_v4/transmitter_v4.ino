#include <formationAlgorithmV3.1.h>
#include "math.h"

//#define distancesF[x] pingDist[x].f
const int pingPin[4] = {6, 7, 9, 10};
const char startKey[4] = {'j','k','l','m'};
...
Formation AlgorithmInstance(20);
/*
struct Coordinate{
  float x;
  float y;  
...
void generateCoordinates(int robotNum, float distances[4])
{
  float Xaverage=0;
  float Yaverage=0;
  for (int i = 0; i < 4; i++)
  ...
  Serial.println(Xaverage);
  Serial.println(Yaverage);
  AlgorithmInstance.m_point[robotNum][0] = Xaverage;
  AlgorithmInstance.m_point[robotNum][1] = Yaverage;  
}
...
void setup() {
  ...
  for (int t = 0; t <= 3; t++)
  {
    Serial1.print(startKey[t]);
    delay(500);
    getDistances(distances);
    Serial.println("distances");
    for (int i = 0; i < 4; i++)
      Serial.println(distances[i]); 
    generateCoordinates(t,distances);
  }
  
  AlgorithmInstance.getShape(4);
  for (int i = 0; i < 4; i++){
    for (int j=0; j < 2; j++)
      Serial.println(AlgorithmInstance.m_point[i][j]);
  }
}
/*for (int i = 0; i < 4; i++){
sendPing(i+1 , pingPin[i]);
